...
# Forward SIGTERM to the current foreground process so docker stop terminates
# cleanly instead of timing out and being force-killed (exit 137).
...
elastalert-create-index --config /opt/elastalert2/config.yaml 2>&1 | grep -v '^WARNING'
